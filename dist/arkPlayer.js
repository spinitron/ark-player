...
const AUDIO_CONTEXT = window.AudioContext || window.webkitAudioContext ? new (window.AudioContext || window.webkitAudioContext)() : null; // Nominal duration (seconds) to calculate next segment URL from previous

const DEFAULT_SEGMENT_DURATION = 300; // Start fetching next segment ahead of end of playback of the current one
...
window.arkPlayer = function (container, options) {
  ...
  try {
    if (typeof AUDIO_CONTEXT !== 'object' || typeof AUDIO_CONTEXT.resume !== 'function' || typeof AUDIO_CONTEXT.state !== 'string') {
      badLuck();
      return;
    }
  } catch (ignore) {
  ...
  const playerPicker = container.querySelector('.ark-player__picker'); // AudioContext time when the Ark was started

  let arkStartedAt = null; // unix millisec in the archive corresponding to arkStartedAt

  let arkStartTime = null; // setInterval timer ID for the monitor

  let monitorInterval; // currentSegment and nextSegment can be a dictionary with
  ...
  let currentSegment = null; // nextSegment may be
  ...
  let nextSegment = null; // on 404, try to download this many next segments
  ...
  (() => {
    ...
    while (hour > stop) {
      ...
      pickerOpts.hours[date].push([hourDate.toLocaleTimeString(...localeStuff).replace(/:00:00 /, ' '), hourDate.toISOString().substring(0, 13).replace(/-/g, '')]);
  ...
  function setupPicker(timestamp) {
    ...
    const seekDateHour = copyOfTimestampParam.slice(0, 11); // Search the date-time picker's database for seekDateHour
    ...
    if (date) {
      ...
      c.querySelector('[name=minutes]').value = `${Math.floor(parseInt(copyOfTimestampParam.slice(11, 13), 10) / 15) * 15}`.padStart(2, '0');
    }
  }
  ...
  function updateDisplay(force) {
    ...
    const hide = 'ark-player_hide'; // Show the player if it was hidden when no start time provided.
  ...
  function monitor() {
    ...
    if (currentSegment && nextSegment === null) {
      const currentEndsAt = currentSegment.started + currentSegment.source.buffer.duration - currentSegment.offset;
    ...
    if (arkStartedAt && AUDIO_CONTEXT.currentTime > arkStartedAt) {
      ...
      playerStatus.querySelector('.ark-player__date').innerHTML = arkTime.toLocaleDateString(...localeStuff).replace(/(^\d\d?\/\d\d?\/)\d\d(\d\d)/, '$1$2');
  ...
  function cueNextSegment({
    ...
    const url = baseUrl.replace('{time}', new Date(segmentTime).toISOString().replace(/[-:]/g, '').replace(/\.\d{3}/, ''));
  ...
  function cueAudio({
    ...
    const startAt = currentSegment === null ? AUDIO_CONTEXT.currentTime + 0.05 : Math.max(AUDIO_CONTEXT.currentTime + 0.01, currentSegment.started + currentSegment.source.buffer.duration - currentSegment.offset);
    nextSegment = {
      segmentTime,
      source,
      offset,
